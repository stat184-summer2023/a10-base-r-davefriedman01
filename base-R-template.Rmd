...
data(iris)
                       ...
                       iris[10, 5]
          as.character(iris[10, 5])
as.factor(as.character(iris[10, 5]))
```
...
plotNormal <- function (repetitions, mean, sd) {
  hist(x = rnorm(n = repetitions, mean = mean, sd = sd))
}
plotNormal(1000, 0, 1)
```
...
plotNormal <- function (repetitions, mean, sd) {
  hist(
    col  = 'blue',
    main = paste('Histogram from a Normal(', mean, ',' , sd, ') Distribution'),
    x    = rnorm(n = repetitions, mean = mean, sd = sd)
  )
}
plotNormal(1000, 0, 1)
```
